...
export class RoutesController {
    ...
    static async create(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            res.status(201).json(ResponseFormatter.success(route, 201));
        } catch (error) { next(error); }
    }
    ...
    static async getOsrmRoute(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (!coordinates || typeof coordinates !== 'string') {
                throw ApiError.badRequest('coordinates query parametresi gerekli');
            }

            // Yalnızca sayılar, virgül, nokta ve noktalı virgüle izin ver
            if (!/^[-\d.,;]+$/.test(coordinates)) {
                throw ApiError.badRequest('Geçersiz koordinat formatı');
            }

            const response = await fetch(`${env.OSRM_URL}/route/v1/driving/${coordinates}?overview=full&geometries=geojson`);
