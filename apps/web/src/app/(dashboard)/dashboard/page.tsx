...
function JobDetailModal({ job, open, onClose, onCompleted }: { job: UpcomingJob | null; open: boolean; onClose: () => void; onCompleted: () => void }) {
    ...
    const handleStatusChange = async (newStatus: string) => {
        setIsSaving(true);
        try {
            await updateJob(job.id, { status: newStatus as 'PENDING' | 'IN_PROGRESS' | 'COMPLETED' | 'CANCELLED' });
            toast.success('Sefer durumu güncellendi.');
            onCompleted();
            onClose();
        } catch {
            ...
            <DialogContent className="sm:max-w-[480px] p-0 overflow-hidden">
                ...
                <div className="p-6">
                    ...
                    <div className="mt-5 pt-4 border-t border-slate-100 space-y-1.5">
                        <div className="text-xs text-slate-400 font-medium">Durumu Güncelle</div>
                        <Select
                            value={status}
